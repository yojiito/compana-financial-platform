...
import { ShieldCheck, CheckCircle2, AlertTriangle, ExternalLink, X, FileText, Building2, Send, Check } from 'lucide-react';
import { UniversalAuditResult } from '@/lib/platform-fact-checker';
import { useLanguage } from '@/lib/language-context';

interface UniversalFactAuditModalProps {
  isOpen: boolean;
  onClose: () => void;
  auditResult: UniversalAuditResult;
}

export default function UniversalFactAuditModal({ isOpen, onClose, auditResult }: UniversalFactAuditModalProps) {
  ...
  const getDomainBadge = () => {
    switch (auditResult.domain) {
      case 'stock':
        return isEn ? 'Listed Stock (EDINET & TSE)' : '東証上場株式 (EDINET & TDnet)';
      case 'unlisted':
        return isEn ? 'Article 440 Gazette Filing' : '会社法第440条 官報決算公告';
      case 'ma':
        return isEn ? 'M&A Timely Disclosure' : '東証適時開示 M&Aディール';
      case 'fund':
        return isEn ? 'Regulatory Fund Filing' : '金融庁届出 機関投資家ファンド';
      case 'reit':
        return isEn ? 'J-REIT Periodic Report' : 'J-REIT 資産運用報告書';
      default:
        return 'Official Verified Record';
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-200 font-sans">
      <div className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="bg-gradient-to-r from-slate-950 via-slate-900 to-teal-950 text-white p-6 relative">
          <button
            onClick={onClose}
            className="absolute top-5 right-5 p-2 text-slate-400 hover:text-white rounded-full bg-slate-800/80 hover:bg-slate-700 transition cursor-pointer"
          >
            <X className="w-5 h-5" />
          </button>
          ...
          <h2 className="text-xl sm:text-2xl font-black text-white">
            {auditResult.entityName} ({auditResult.entityId})
          </h2>
          <p className="text-xs sm:text-sm text-slate-300 mt-1">
            {isEn
              ? 'Independent automated data audit verifying official government/regulatory disclosures and mathematical balance.'
              : '金融庁EDINET・国立印刷局官報・東証適時開示等の原本に基づき、数値・固有名詞の整合性を自動監査済。'}
          </p>
        </div>
        ...
        <div className="p-6 overflow-y-auto space-y-6 text-slate-800 text-xs sm:text-sm">
          ...
          <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200/80 space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-bold text-slate-500 flex items-center gap-1.5">
                ...
                {isEn ? 'Official Regulatory Filing Source' : '公式準拠 開示資料元'}
              </span>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 pt-1 text-[11px] text-slate-600">
              ...
              <div>
                <span className="text-slate-400 block">{isEn ? 'Filing Benchmark' : '開示基準期'}</span>
                <span className="font-medium text-slate-800">{auditResult.sourceMeta.filingDate}</span>
              </div>
              <div>
                <span className="text-slate-400 block">{isEn ? 'Filing Type' : '開示種別'}</span>
                <span className="font-medium text-slate-800">{auditResult.sourceMeta.filingType}</span>
              </div>
            </div>
          ...
          <div className="flex items-center justify-between p-3 bg-teal-50/50 rounded-2xl border border-teal-100">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-teal-900">
                {isEn ? 'Inspect Regulatory Source Authority' : '提出・監督機関で原本記録を確認'}
              </span>
            </div>
            <a
              href={auditResult.sourceMeta.officialSourceUrl}
              target="_blank"
            ...
            >
              <span>{isEn ? 'Open Official Database' : '公式原本を見る'}</span>
          ...
          <div className="border-t border-slate-100 pt-4 space-y-2">
            <span className="text-[11px] font-bold text-slate-500 block">
              {isEn ? 'Found an update or discrepancy? Report to Data Team:' : '最新の開示変更・修正をお気づきですか？データ監査チームへ報告:'}
            </span>
            {isReportSubmitted ? (
              ...
              <form onSubmit={handleReportSubmit} className="flex gap-2">
                <input
                  type="text"
                  placeholder={isEn ? 'e.g., New quarterly report filed, subsidiary acquisition...' : '例: 四半期報告書が提出された、代表者変更等...'}
                  value={reportText}
                ...
                <button
                  ...
                  className="px-4 py-2 bg-slate-900 hover:bg-teal-700 disabled:opacity-50 text-white rounded-xl text-xs font-bold flex items-center gap-1.5 transition shrink-0 cursor-pointer"
                >
