...
export const MAJOR_10YR_FINANCIALS: Record<string, any[]> = {
  // 🚗 トヨタ自動車 (7203)
  '7203': [
...
// 📑 最新IR決算短信サマリー
export const SAMPLE_IR_SUMMARIES = [
  {
    tickerCode: '7203',
...
async function main() {
  console.log('================================================================');
  console.log('🚀 SEEDING 10-YEAR FINANCIALS & IR DISCLOSURES ACROSS ALL LISTED COMPANIES');
  console.log('================================================================\n');

  // 1. 主要銘柄の正確な10年財務データをUpsert
  for (const [ticker, finList] of Object.entries(MAJOR_10YR_FINANCIALS)) {
    ...
    console.log(`✅ [${ticker}] ${comp.name} 10-year financials synchronized!`);
  }

  // 2. 全上場企業（3,903社）に対して、過去10年の標準推移データを補完
  ...
  console.log(`\nSynthesizing 10-year financials for ${allCompanies.length} companies without history...`);
  ...
  for (const ir of SAMPLE_IR_SUMMARIES) {
    ...
    console.log(`📑 [${ir.tickerCode}] ${comp.name} IR Summary & Disclosures seeded!`);
  }

  console.log('\n🎉 ALL 10-YEAR FINANCIALS & IR SUMMARIES SEEDED SUCCESSFULLY!');
}
